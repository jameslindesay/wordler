...
@timer_func
def score_wordlist(colour_combinations, list_possible_words_before, valid_words):
    scores = [fully_score_word(word, colour_combinations, list_possible_words_before) for word in tqdm(list_possible_words_before, desc="Scoring...")]
    return pd.DataFrame({'word':list_possible_words_before, 'score':scores})
...
def main():
    ...
    guesses = 0
    while guesses < 6:
        scored_words = score_wordlist(colour_combinations, list_possible_words_before, valid_words)
        print(scored_words.sort_values('score', ascending=False).head(3))
        print("Remaining : " + str(round(np.log2(len(list_possible_words_before)), 2)))
